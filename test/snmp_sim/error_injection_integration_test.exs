defmodule SnmpSim.ErrorInjectionIntegrationTest do
  ...
  describe "Error injection integration" do
    test "ErrorInjector can communicate with Device", %{
    ...
    test "Device processes error injection messages", %{
      device_pid: device_pid,
      device_config: config
    } do
      {:ok, injector_pid} = ErrorInjector.start_link(device_pid, config.port)
    ...
    test "SNMP error injection", %{device_pid: device_pid, device_config: config} do
      ...
      # Inject SNMP errors
      :ok =
        ErrorInjector.inject_snmp_error(injector_pid, :genErr,
          probability: 0.2,
  ...
  describe "Test scenario integration" do
    ...
    test "Signal degradation scenario", %{device_pid: device_pid} do
      ...
      result =
        TestScenarios.signal_degradation_scenario(devices,
          snr_degradation: 5,
          duration_minutes: 1,
          pattern: :steady
        )
  ...
  describe "Error injection persistence" do
    test "Clear all errors removes all conditions", %{
      device_pid: device_pid,
      device_config: config
    } do
      {:ok, injector_pid} = ErrorInjector.start_link(device_pid, config.port)
  ...
  describe "Scenario validation" do
    test "Scenario with empty device list", %{} do
      ...
      result =
        TestScenarios.network_outage_scenario([],
          duration_seconds: 30
        )
