  ...
  describe "SNMP protocol compliance" do
    ...
    test "SNMP GETBULK walk terminates correctly", %{port: port} do
      ...
      {output, exit_code} = System.cmd("timeout", [
        "30s", "snmpbulkwalk", "-On","-v2c", "-c", "public",
    ...
    test "SNMP GETNEXT progression works correctly", %{port: port} do
      ...
      {_final_oid, visited_oids} = Enum.reduce_while(1..max_steps, {start_oid, MapSet.new()}, fn _i, {current_oid, visited} ->
        {output, exit_code} = System.cmd("snmpgetnext", [
          "-On","-v2c", "-c", "public", "127.0.0.1:#{port}", current_oid
        ], stderr_to_stdout: true)

        if exit_code != 0 do
          {:halt, {current_oid, visited}}
        else
          # Extract the returned OID from output
          case Regex.run(~r/^\.([0-9.]+) =/, output) do
            [_, returned_oid] ->
              # Check for loops
              if MapSet.member?(visited, returned_oid) do
                flunk("GETNEXT loop detected: OID #{returned_oid} returned twice")
              end

              new_visited = MapSet.put(visited, returned_oid)
              {:cont, {returned_oid, new_visited}}
