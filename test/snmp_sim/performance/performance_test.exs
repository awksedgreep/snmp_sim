defmodule SnmpSim.Performance.PerformanceTest do
  ...
  # Test configuration
  # 10 minutes for long-running tests
  @performance_timeout 600_000
  ...
  describe "Large Scale Performance Tests" do
    ...
    test "sustains 100K+ requests/second throughput" do
      ...
      result =
        Benchmarks.run_single_benchmark("high_throughput_test",
          device_ports: device_ports,
          concurrent_clients: 200,
          # Target 50K req/s (will scale up)
          request_rate: 50_000,
          # 2 minutes
          duration: 120_000
        )
    ...
    test "maintains memory usage under 1GB for 10K devices" do
      ...
      load_test_task =
        Task.async(fn ->
          # 3 minutes
          test_sustained_load(sample_ports, 180_000)
        end)
      ...
      Logger.info(
        "Memory analysis - Initial: #{initial_memory}MB, Final: #{final_memory}MB, Peak: #{peak_memory}MB"
      )

      # Validate memory requirements
      # Under 1GB peak usage
    ...
    test "achieves sub-5ms response times for cached lookups" do
      ...
      Logger.info(
        "Response time analysis - Avg: #{avg_response_time}ms, P95: #{p95_response_time}ms"
      )
      ...
      # Over 85% cache hit ratio
      assert cache_hit_ratio > 85.0
    end
  end
  ...
  describe "Resource Management Tests" do
    ...
    test "automatically cleans up idle devices" do
      ...
      :ok =
        ResourceManager.update_limits(
          # 5 seconds
          idle_threshold: 5_000,
          # 2 seconds
          cleanup_interval: 2_000
        )
  ...
  describe "Scaling and Efficiency Tests" do
    ...
    test "scales performance linearly with device count" do
      ...
      assert scaling_efficiency.linear_regression_r2 > 0.8
      # Less than 20% degradation
      assert scaling_efficiency.efficiency_degradation < 20
    ...
    test "maintains stable performance over 24+ hour simulation" do
      ...
      assert stability_result.performance_variance < 10
  ...
  defp create_devices_gradually(count, start_port) do
    ...
    Enum.reduce(1..batches, [], fn batch_num, acc ->
      ...
      if batch_start <= start_port + count - 1 do
        ...
        created_ports =
          Enum.filter(batch_ports, fn port ->
  ...
  defp create_devices_up_to_limit(attempt_count, start_port) do
    Enum.reduce(1..attempt_count, [], fn i, acc ->
      ...
      case OptimizedDevicePool.get_device(port) do
        ...
        # Hit the limit
        {:error, :resource_limit_exceeded} -> acc
  ...
  defp test_scaling_performance(device_ports) do
    ...
    result =
      Benchmarks.run_single_benchmark("scaling_test",
        device_ports: device_ports,
  ...
  defp analyze_scaling_efficiency(scaling_results) do
    ...
    efficiency_scores =
      Enum.map(scaling_results, fn {count, result} ->
  ...
  defp simulate_daily_load_patterns(device_ports, duration_ms) do
    ...
    simulate_load_loop(
      ...
      start_time,
      end_time,
      {performance_samples, memory_samples, error_counts}
    )
  end

  defp simulate_load_loop(
         device_ports,
         start_time,
         end_time,
         {perf_samples, mem_samples, error_counts}
       ) do
    current_time = System.monotonic_time(:millisecond)
    ...
    if current_time < end_time do
      ...
      simulate_load_loop(
        ...
        start_time,
        end_time,
        {new_perf_samples, new_mem_samples, new_error_counts}
      )
    else
      # Analyze results
      %{
