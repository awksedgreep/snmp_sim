defmodule SnmpSim.BulkOperationsTest do
  use ExUnit.Case, async: false
  
  alias SnmpSim.{BulkOperations, OIDTree}
  ...
  describe "GETBULK Request Handling" do
    setup do
      ...
      tree = OIDTree.new()
             ...
             |> OIDTree.insert("1.3.6.1.2.1.1.3.0", {:timeticks, 123456})
             |> OIDTree.insert("1.3.6.1.2.1.2.1.0", 3)  # ifNumber
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.1.1", 1)  # ifIndex.1
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.1.2", 2)  # ifIndex.2
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.1.3", 3)  # ifIndex.3
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.2.1", "eth0")  # ifDescr.1
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.2.2", "eth1")  # ifDescr.2
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.2.3", "lo0")   # ifDescr.3
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.10.1", {:counter32, 1000})  # ifInOctets.1
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.10.2", {:counter32, 2000})  # ifInOctets.2
             |> OIDTree.insert("1.3.6.1.2.1.2.2.1.10.3", {:counter32, 3000})  # ifInOctets.3
    ...
    test "handles GETBULK with non-repeaters", %{tree: tree} do
      varbinds = [
        {"1.3.6.1.2.1.1.1.0", nil},  # Non-repeater: sysDescr
        {"1.3.6.1.2.1.2.2.1.1", nil}  # Repeater: ifIndex
      ]
      ...
      assert [
        {"1.3.6.1.2.1.1.3.0", {:timeticks, 123456}, nil},  # Non-repeater result
        {"1.3.6.1.2.1.2.2.1.1.1", 1, nil},  # First repeater
        {"1.3.6.1.2.1.2.2.1.1.2", 2, nil},  # Second repeater
        {"1.3.6.1.2.1.2.2.1.1.3", 3, nil}   # Third repeater
      ] = results
    end
    
    test "handles GETBULK with multiple repeating variables", %{tree: tree} do
      varbinds = [
        {"1.3.6.1.2.1.2.2.1.1", nil},   # ifIndex
        {"1.3.6.1.2.1.2.2.1.2", nil}    # ifDescr
      ]
      ...
      assert [
        {"1.3.6.1.2.1.2.2.1.1.1", 1, nil},      # ifIndex.1
        {"1.3.6.1.2.1.2.2.1.1.2", 2, nil},      # ifIndex.2
        {"1.3.6.1.2.1.2.2.1.2.1", "eth0", nil}, # ifDescr.1
        {"1.3.6.1.2.1.2.2.1.2.2", "eth1", nil}  # ifDescr.2
      ] = results
    end
    
    test "handles GETBULK at end of tree", %{tree: tree} do
      varbinds = [{"1.3.6.1.2.1.2.2.1.10.2", nil}]  # Near end of tree
    ...
    test "handles GETBULK beyond end of tree", %{tree: tree} do
      varbinds = [{"1.3.6.1.9.9.9", nil}]  # Beyond all OIDs
  ...
  describe "Response Size Management" do
    test "estimates response size for various data types" do
      varbinds = [
        ...
        {"1.3.6.1.2.1.1.3.0", {:timeticks, 123456}, nil},
        {"1.3.6.1.2.1.2.2.1.10.1", {:counter32, 1234567890}, nil},
        {"1.3.6.1.2.1.2.2.1.11.1", {:counter64, 1234567890123456}, nil}
      ]
      
      size = BulkOperations.estimate_response_size(varbinds)
      
      # Should be reasonable estimate (not exact, but in right ballpark)
      assert size > 100   # Minimum overhead
      assert size < 500   # Not excessive for small response
    end
    
    test "optimize_bulk_response fits results within size limit" do
      # Create large results that exceed size limit
      large_results = for i <- 1..100 do
        oid = "1.3.6.1.2.1.2.2.1.10.#{i}"
        value = "Very long interface description string for interface #{i} that makes the response large"
        {oid, value, nil}
      end
      ...
      assert length(optimized) > 0  # But should have at least some results
  ...
  describe "Interface Table Processing" do
    setup do
      ...
      tree = for interface <- interfaces, {col_id, col_name} <- columns, reduce: tree do
        acc -> 
          oid = "1.3.6.1.2.1.2.2.1.#{col_id}.#{interface}"
          value = case col_name do
            "Index" -> interface
  ...
  describe "Edge Cases and Error Handling" do
    ...
    test "handles max non-repeaters equal to varbinds length" do
      ...
      assert [
        {"1.3.6.1.2.1.1.3.0", "sysUpTime", nil},  # Next after sysDescr
        {"1.3.6.1.2.1.1.3.0", "sysUpTime", nil}   # Next after non-existent OID
      ] = results
    end
    ...
    test "estimates size for various SNMP data types" do
      ...
      test_cases = [
        "Simple string",
        42,
        {:counter32, 1234567890},
        {:counter64, 1234567890123456789},
        {:gauge32, 50},
        {:timeticks, 123456},
