defmodule SnmpSim.ValueSimulatorPhase5Test do
  use ExUnit.Case, async: false
  alias SnmpSim.ValueSimulator
  ...
  describe "counter wrapping functionality" do
    ...
    test "time_until_counter_wrap calculates correctly" do
      # Near 32-bit max
      current_value = 4_000_000_000
      # 1000 per second
      increment_rate = 1000

      time_until_wrap =
        ValueSimulator.time_until_counter_wrap(
          current_value,
          increment_rate,
          "Counter32"
        )
    ...
    test "apply_device_specific_counter_behavior varies by device type" do
      ...
      cmts_result =
        ValueSimulator.apply_device_specific_counter_behavior(
          ...
          "Counter32",
          :cmts,
          %{synchronized_wrap: true, sync_boundary: 1000}
        )
    ...
    test "handle_counter_discontinuity detects wraps" do
      ...
      result =
        ValueSimulator.handle_counter_discontinuity(
          old_value,
          new_value,
  ...
  describe "configurable jitter and variance" do
      ...
      uniform_samples =
        for _ <- 1..50 do
          {:counter32, value} =
            ValueSimulator.simulate_value(
              profile_data,
              {:traffic_counter, config_uniform},
              device_state
            )
      ...
      gaussian_samples =
        for _ <- 1..50 do
          {:counter32, value} =
            ValueSimulator.simulate_value(
              profile_data,
              {:traffic_counter, config_gaussian},
              device_state
            )
    ...
    test "time_correlated variance changes over time" do
      ...
      config = %{
        ...
        variance: 0.2,
        # 5 minute period
        correlation_period_seconds: 300,
        rate_range: {1000, 2000}
      }
      ...
      # Some variation
      assert length(unique_results) >= 3
      # Not too much variation (allow for randomness)
      assert length(unique_results) <= 10
    end
    ...
    test "device_specific variance reflects device characteristics" do
      ...
      mta_variance =
        Enum.reduce(mta_samples, 0, fn x, acc ->
          acc + :math.pow(x - mta_mean, 2)
        end) / length(mta_samples)

      server_variance =
        Enum.reduce(server_samples, 0, fn x, acc ->
  ...
  describe "integration with time patterns and correlations" do
    ...
    test "error counter includes correlation with utilization" do
      ...
      device_state = %{
        # 30 minutes
        uptime: 1800,
        # High utilization should increase errors
        interface_utilization: 0.8,
        # Lower signal quality should increase errors
        signal_quality: 0.7,
        device_type: :cable_modem
      }
