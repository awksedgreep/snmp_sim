defmodule SnmpSim.GetBulkOidFormatTest do
  ...
  describe "SNMP GETBULK OID format fix verification" do
    test "SNMP GETBULK works correctly after OID format fix" do
      ...
      {output, exit_code} = System.cmd("snmpbulkwalk", [
        "-v2c",
        "-c", "public",
        "-r", "1",
        "-t", "3",
      ...
      assert String.contains?(output, "Motorola SB6141"), "Expected device description in output: #{output}"
      ...
      valid_data_lines = Enum.filter(lines, fn line -> 
        String.contains?(line, "STRING:") || 
        String.contains?(line, "INTEGER:") || 
        String.contains?(line, "Wrong Type")  # sysObjectID has this issue but it's still valid data
      end)
      ...
      {get_output, get_exit_code} = System.cmd("snmpget", [
        "-v2c",
        "-c", "public",
        "-r", "1",
        "-t", "3",
    ...
    test "SNMP GETBULK handles different starting OIDs correctly" do
      ...
      test_oids = [
        "1.3.6.1",               # Root OID - should redirect to system
        "1.3.6.1.2.1.1",         # System group - should redirect to system
        "1.3.6.1.2.1.1.1.0"      # System description - should return next OID
      ]
