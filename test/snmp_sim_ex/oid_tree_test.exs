defmodule SnmpSimEx.OIDTreeTest do
  ...
  describe "OID Tree Creation and Basic Operations" do
    ...
    test "inserts multiple OIDs" do
      ...
      assert {:ok, "System Description", nil} = OIDTree.get(tree, "1.3.6.1.2.1.1.1.0")
      assert {:ok, {:timeticks, 123456}, nil} = OIDTree.get(tree, "1.3.6.1.2.1.1.3.0")
    ...
    test "inserts OID with behavior information" do
      ...
      tree = OIDTree.insert(tree, "1.3.6.1.2.1.2.2.1.10.1", {:counter32, 1234567}, behavior)
      
      assert {:ok, {:counter32, 1234567}, behavior} = OIDTree.get(tree, "1.3.6.1.2.1.2.2.1.10.1")
    end
    ...
    test "overwrites existing OID value" do
      ...
      assert OIDTree.size(tree) == 1  # Size shouldn't change
  ...
  describe "GETBULK Operations" do
    setup do
      ...
      tree = Enum.reduce(interface_oids, tree, fn {oid, value}, acc ->
  ...
  describe "Performance with Large OID Trees" do
    test "handles large OID trees efficiently" do
      ...
      tree = Enum.reduce(1..100, tree, fn interface_index, acc ->
        # Add multiple columns for each interface
        columns = [1, 2, 10, 16, 14, 20]  # Common interface table columns
      ...
      assert OIDTree.size(tree) == 600  # 100 interfaces * 6 columns
    ...
    test "GETNEXT traversal is efficient on large trees" do
      ...
      tree = Enum.reduce(1..50, tree, fn interface_index, acc ->
        oid = "1.3.6.1.2.1.2.2.1.10.#{interface_index}"  # ifInOctets
