defmodule SnmpSim.WalkParserTest do
  ...
  describe "Walk File Parsing" do
    test "parses named MIB format walk files (IF-MIB::ifInOctets.2)" do
      ...
      assert {"1.3.6.1.2.1.2.2.1.10.2", %{type: "Counter32", value: 1234567890, mib_name: "IF-MIB::ifInOctets.2"}} = result
    end
    ...
    test "parses numeric OID format walk files (.1.3.6.1.2.1.2.2.1.10.2)" do
      ...
      assert {"1.3.6.1.2.1.2.2.1.10.2", %{type: "Counter32", value: 1234567890}} = result
    end

    test "handles mixed walk file formats in same file" do
      temp_dir = System.tmp_dir!()
      temp_file = Path.join(temp_dir, "mixed_walk_#{:rand.uniform(1000000)}")
    ...
    test "extracts data types correctly (Counter32, STRING, INTEGER)" do
      ...
      assert {_oid3, %{type: "Counter32", value: 1234567890}} = Enum.at(results, 2)
      assert {_oid4, %{type: "Gauge32", value: 1000000000}} = Enum.at(results, 3)
    end
    ...
    test "handles timeticks format correctly" do
      ...
      assert {"1.3.6.1.2.1.1.3.0", %{type: "Timeticks", value: 12345600}} = result
    end
  end
  ...
  describe "File Reading" do
    test "reads actual walk file successfully" do
      ...
      assert Map.has_key?(oid_map, "1.3.6.1.2.1.1.1.0")  # sysDescr
      assert Map.has_key?(oid_map, "1.3.6.1.2.1.2.1.0")  # ifNumber
    end
