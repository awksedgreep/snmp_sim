defmodule SnmpSim.CorrelationEngineTest do
  ...
  describe "apply_correlations/5" do
    test "applies correlations between related metrics" do
      ...
      updated_state = CorrelationEngine.apply_correlations(
  ...
  describe "get_device_correlations/1" do
    test "returns cable modem correlations" do
      ...
      signal_throughput = Enum.find(correlations, fn
    ...
    test "returns MTA correlations" do
      ...
      signal_jitter = Enum.find(correlations, fn
    ...
    test "returns switch correlations" do
      ...
      cpu_temp = Enum.find(correlations, fn
    ...
    test "returns router correlations" do
      ...
      cpu_routing = Enum.find(correlations, fn
    ...
    test "returns CMTS correlations" do
      ...
      modems_cpu = Enum.find(correlations, fn
    ...
    test "returns server correlations" do
      ...
      cpu_memory = Enum.find(correlations, fn
    ...
    test "returns generic correlations for unknown device types" do
      ...
      basic_correlation = Enum.find(correlations, fn
  ...
  describe "calculate_signal_throughput_correlation/3" do
    test "calculates throughput based on excellent signal quality" do
      ...
      max_throughput = 100_000_000  # 100 Mbps
      
      result = CorrelationEngine.calculate_signal_throughput_correlation(
        snr_db, 
        power_level_dbmv, 
        max_throughput
      )
      ...
      assert result <= max_throughput * 1.1  # Allow for jitter
    end

    test "reduces throughput for poor signal quality" do
      snr_db = 15.0  # Poor SNR
      power_level_dbmv = -12.0  # Poor power level
    ...
    test "handles marginal signal conditions" do
      snr_db = 25.0  # Marginal SNR
      power_level_dbmv = 8.0  # Marginal power level
      max_throughput = 100_000_000
  ...
  describe "calculate_utilization_error_correlation/2" do
    test "increases error rates with higher utilization" do
      low_util_errors = CorrelationEngine.calculate_utilization_error_correlation(
        20.0, 
        :ethernet_gigabit
      )
      
      high_util_errors = CorrelationEngine.calculate_utilization_error_correlation(
        90.0, 
        :ethernet_gigabit
      )
      ...
      assert high_util_errors <= 0.1  # Capped at 10%
    end

    test "applies different base error rates by interface type" do
      utilization = 50.0
      
      gigabit_errors = CorrelationEngine.calculate_utilization_error_correlation(
        utilization, 
        :ethernet_gigabit
      )
      
      wifi_errors = CorrelationEngine.calculate_utilization_error_correlation(
        utilization, 
        :wifi
      )
      
      docsis_errors = CorrelationEngine.calculate_utilization_error_correlation(
        utilization, 
        :docsis
      )
    ...
    test "caps error rates at reasonable maximum" do
      ...
      max_errors = CorrelationEngine.calculate_utilization_error_correlation(
        100.0, 
        :wifi
      )
      
      assert max_errors <= 0.1  # Should not exceed 10%
    end
  end
  ...
  describe "calculate_temperature_performance_correlation/2" do
    test "calculates impact for cable modem at optimal temperature" do
      result = CorrelationEngine.calculate_temperature_performance_correlation(
        25.0, 
        :cable_modem
      )
    ...
    test "handles different equipment types appropriately" do
      ...
      cable_modem_result = CorrelationEngine.calculate_temperature_performance_correlation(
        high_temp, 
        :cable_modem
      )
      
      server_result = CorrelationEngine.calculate_temperature_performance_correlation(
        high_temp, 
        :server
      )
  ...
  describe "calculate_power_consumption_correlation/2" do
    test "calculates power based on device metrics" do
      ...
      result = CorrelationEngine.calculate_power_consumption_correlation(
        device_metrics, 
        :cable_modem
      )
      ...
      assert result >= 12.0
      assert result <= 25.0  # Reasonable upper bound
    end
    ...
    test "varies power consumption by device type" do
      ...
      cable_modem_power = CorrelationEngine.calculate_power_consumption_correlation(
        device_metrics, 
        :cable_modem
      )
      
      server_power = CorrelationEngine.calculate_power_consumption_correlation(
        device_metrics, 
        :server
      )
      
      cmts_power = CorrelationEngine.calculate_power_consumption_correlation(
        device_metrics, 
        :cmts
      )
      ...
      assert cmts_power > server_power  # CMTS is largest equipment
    end
    ...
    test "increases power with CPU and network activity" do
      ...
      low_power = CorrelationEngine.calculate_power_consumption_correlation(
        low_activity_metrics, 
        :switch
      )
      
      high_power = CorrelationEngine.calculate_power_consumption_correlation(
        high_activity_metrics, 
        :switch
      )
    ...
    test "includes cooling power for high temperatures" do
      ...
      normal_power = CorrelationEngine.calculate_power_consumption_correlation(
        normal_temp_metrics, 
        :router
      )
      
      high_temp_power = CorrelationEngine.calculate_power_consumption_correlation(
        high_temp_metrics, 
        :router
      )
  ...
  describe "integration and performance" do
    test "correlations work together without conflicts" do
      ...
      Enum.each(updated_state2, fn {_key, value} ->
        ...
        assert value >= 0 or is_float(value)  # Allow negative floats for some metrics
      end)
    end
    ...
    test "signal throughput calculations handle edge cases" do
      # Test very low SNR
      low_result = CorrelationEngine.calculate_signal_throughput_correlation(
        5.0, 
        -20.0, 
        100_000_000
      )
      ...
      # Test very high SNR
      high_result = CorrelationEngine.calculate_signal_throughput_correlation(
        45.0, 
        15.0, 
        100_000_000
      )
      ...
      assert low_result < 50_000_000  # Should be significantly reduced
      assert high_result >= 60_000_000  # Should be reasonably high (allow for jitter)
    end
  end
end