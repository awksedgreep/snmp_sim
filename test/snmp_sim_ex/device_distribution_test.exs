defmodule SNMPSimEx.DeviceDistributionTest do
  ...
  describe "port assignment building" do
    test "builds assignments from device mix" do
      ...
      assert device_a_count >= 10  # May be larger if non-contiguous
    ...
    test "raises error when not enough ports" do
      ...
      port_range = 1000..1010  # Only 11 ports
  ...
  describe "density statistics" do
    test "calculates density statistics correctly" do
      assignments = %{
        type_a: 1000..1099,  # 100 devices
        type_b: 1100..1149,  # 50 devices
        type_c: 1150..1154   # 5 devices
      }
  ...
  describe "device characteristics" do
    ...
    test "handles unknown device types gracefully" do
      ...
      assert unknown.error_rates.high == 1.0  # High error rate for unknown
    end
  end
  ...
  describe "device ID generation" do
    ...
    test "generates MAC-based device IDs" do
      ...
      assert String.contains?(id, "CM")  # Cable modem type code
      assert is_binary(id)
    end
    ...
    test "generates hostname-based device IDs" do
      ...
      assert String.contains?(id, "039500")  # Padded port number
    end
    ...
    test "generates serial-based device IDs" do
      ...
      assert String.length(id) > 10  # Should be reasonably long
    end
  end
  ...
  describe "port assignment validation" do
    ...
    test "detects overlapping port assignments" do
      overlapping_assignments = %{
        type_a: 1000..1150,
        type_b: 1100..1200  # Overlaps with type_a
      }
    ...
    test "validates reasonable distribution sizes" do
      ...
      huge_assignments = %{
        type_a: 1..200_000  # Too many devices
      }
    ...
    test "validates edge cases" do
      single_port = %{
        type_a: 1000..1000  # Single port range
      }
      assert DeviceDistribution.validate_port_assignments(single_port) == :ok
      
      adjacent_ranges = %{
        type_a: 1000..1100,
        type_b: 1101..1200  # Adjacent but not overlapping
      }
