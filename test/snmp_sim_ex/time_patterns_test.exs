defmodule SnmpSimEx.TimePatternsTest do
  ...
  describe "Daily Utilization Patterns" do
    ...
    test "returns high utilization during business hours" do
      business_hours = ~U[2024-01-15 14:00:00Z]  # 2 PM
    ...
    test "returns peak utilization during evening hours" do
      evening_peak = ~U[2024-01-15 19:00:00Z]  # 7 PM
      
      factor = TimePatterns.get_daily_utilization_pattern(evening_peak)
      
      assert factor >= 1.2
      assert factor <= 1.8  # Can include bursts
    end
  ...
  describe "Weekly Pattern Variations" do
    test "returns full pattern for weekdays" do
      tuesday = ~U[2024-01-16 14:00:00Z]  # Tuesday at 2 PM
    ...
    test "returns reduced pattern for Saturday" do
      saturday = ~U[2024-01-20 14:00:00Z]  # Saturday at 2 PM
    ...
    test "returns lowest pattern for Sunday" do
      sunday = ~U[2024-01-21 14:00:00Z]  # Sunday at 2 PM
  ...
  describe "Temperature Patterns" do
    test "returns seasonal temperature variations" do
      winter = ~U[2024-01-15 12:00:00Z]  # January (winter)
      summer = ~U[2024-07-15 12:00:00Z]  # July (summer)
    ...
    test "returns daily temperature variations" do
      early_morning = ~U[2024-01-15 06:00:00Z]  # 6 AM (coldest)
      afternoon = ~U[2024-01-15 15:00:00Z]      # 3 PM (warmest)
  ...
  describe "Interface Traffic Rates" do
    test "returns appropriate rates for different interface types" do
      ...
      assert eth_max == 125_000_000  # 125 MB/s
    ...
    test "applies time-based factors to traffic rates" do
      business_hour = ~U[2024-01-15 14:00:00Z]  # Tuesday 2 PM
      late_night = ~U[2024-01-15 03:00:00Z]     # Tuesday 3 AM
      
      {_, _, business_factor} = TimePatterns.get_interface_traffic_rate(:ethernet_gigabit, business_hour)
      {_, _, night_factor} = TimePatterns.get_interface_traffic_rate(:ethernet_gigabit, late_night)
