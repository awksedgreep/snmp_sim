defmodule SnmpSimExErlangSnmpIntegrationTest do
  ...
  describe "Integration with Erlang SNMP Manager" do
    ...
    test "Erlang SNMP handles lexicographic ordering correctly", %{target_name: target_name, user_id: user_id} do
    ...
    test "Erlang SNMP can walk large interface tables efficiently", %{target_name: target_name, user_id: user_id} do
    ...
    test "Erlang SNMP handles invalid community strings", %{user_id: user_id, port: port} do
      ...
      bad_agent_config = [{:community, ~c"invalid"},
                          {:engine_id, ~c"test_engine"},
                          {:version, :v2},
                          {:sec_model, :v2c},
                          {:sec_name, ~c"invalid"},
      ...
      case :snmpm.register_agent(user_id, {127, 0, 0, 1}, port, bad_agent_config) do
        :ok -> :ok
        {:error, {:already_registered, _}} -> :ok  # Target already registered
      end
    ...
    test "Erlang SNMP concurrent requests work properly", %{target_name: target_name, user_id: user_id} do
      ...
      successful_count = Enum.count(results, fn
        ...
        {:error, :not_found} -> true  # Acceptable for integration test
        _ -> false
      end)
    ...
    test "Erlang SNMP handles counter values correctly", %{target_name: target_name, user_id: user_id} do
      # Test counter-type OIDs (should be numeric)
      case :snmpm.sync_get2(user_id, target_name, [[1,3,6,1,2,1,2,2,1,10,1]]) do  # ifInOctets
    ...
    test "Erlang SNMP can retrieve string values correctly", %{target_name: target_name, user_id: user_id} do
      ...
      for oid <- string_oids do
        case :snmpm.sync_get2(user_id, target_name, [oid]) do
          {:ok, {:pdu, _, _, 0, 0, [{:varbind, ^oid, _type, value, _status}]}, _} when is_binary(value) or is_list(value) ->
    ...
    test "Erlang SNMP handles SNMPv1 vs v2c correctly", %{user_id: user_id, port: port} do
      ...
      for version <- [:v1, :v2] do
        version_config = case version do
          :v1 -> [{:community, ~c"public"}, {:engine_id, ~c"test_engine"}, {:version, :v1}, {:sec_model, :v1}, {:sec_name, ~c"public"}, {:sec_level, :noAuthNoPriv}]
          :v2 -> [{:community, ~c"public"}, {:engine_id, ~c"test_engine"}, {:version, :v2}, {:sec_model, :v2c}, {:sec_name, ~c"public"}, {:sec_level, :noAuthNoPriv}]
        end
        
        case :snmpm.register_agent(user_id, {127, 0, 0, 1}, port, version_config) do
          :ok -> :ok
          {:error, {:already_registered, _}} -> :ok  # Target already registered with this config
        end
  ...
  describe "Protocol Compliance and Edge Cases" do
    setup do
      ...
      # Register a user with the SNMP manager
      user_id = "compliance_user_#{:rand.uniform(100000)}"
      case :snmpm.register_user(user_id, :snmpm_user_default, []) do
        :ok -> :ok
        {:error, {:already_registered, _}} -> :ok
      end
    ...
    test "Erlang SNMP validates SNMP packet structure", %{target_name: target_name, user_id: user_id} do
    ...
    test "Erlang SNMP validates BER encoding", %{target_name: target_name, user_id: user_id} do
      ...
      test_oids = [
        ...
        [1,3,6,1,2,1,2,1,0],      # Integer
        [1,3,6,1,2,1,2,2,1,10,1]  # Counter (if exists)
      ]
