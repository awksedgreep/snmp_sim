defmodule SnmpSim.TestHelpers.PerformanceHelper do
  ...
  def run_sustained_load_test(devices, target_rps, duration_ms, options \\ %{}) do
    ...
    monitoring_tasks = if monitor_response_times do
      [Task.async(fn -> collect_response_times(devices, target_rps, duration_ms) end) | monitoring_tasks]
    else
      monitoring_tasks
    end
    
    monitoring_tasks = if monitor_resource_usage do
    ...
    monitoring_data = case monitoring_results do
  ...
  """
  def analyze_response_times(response_times) when is_list(response_times) and length(response_times) > 0 do
  ...
  def measure_scalability(base_device_count, max_device_count, step_size, test_duration_ms) do
    ...
    Enum.map(device_counts, fn device_count ->
      ...
      results = run_sustained_load_test(
        devices,
        100,  # Fixed RPS for scalability testing
        test_duration_ms,
        %{
          monitor_response_times: true,
          monitor_resource_usage: true
      ...
      # Memory efficiency
      memory_per_device = if length(results.memory_samples) > 0 do
  ...
  defp execute_load_loop(devices, interval_ms, end_time, request_count, errors) do
    ...
    if current_time >= end_time do
      total_duration_seconds = (current_time - (end_time - System.monotonic_time(:millisecond))) / 1000
      actual_rps = if total_duration_seconds > 0, do: request_count / total_duration_seconds, else: 0
  ...
  defp collect_response_times_loop(devices, interval_ms, end_time, response_times) do
    ...
    if current_time >= end_time do
      ...
      {response_time, _result} = measure_response_time(fn ->
        Device.get(device, "1.3.6.1.2.1.1.1.0")
      end)
