defmodule SnmpSim.TestHelpers.StabilityTestHelper do
  ...
  def run_load_test(devices, target_rps, duration_ms, options \\ %{}) do
    ...
    monitoring_tasks = start_load_test_monitoring(
      duration_ms,
  ...
  def run_endurance_test(duration_ms, options \\ %{}) do
    ...
    monitoring_data = if monitor_everything do
    ...
    results = execute_endurance_cycles(
      workload_patterns,
      start_time,
      end_time,
      inject_failures,
    ...
    # Finalize monitoring
    final_monitoring = if monitor_everything do
  ...
  def check_system_health do
    ...
    memory_usage_ratio = memory_info[:total] / (1024 * 1024 * 1024)  # Convert to GB
  ...
  def cleanup_all do
    ...
    test_processes = Process.registered()
  ...
  defp start_load_test_monitoring(duration_ms, monitor_response_times, monitor_error_rates, monitor_resource_usage, monitor_process_counts) do
    tasks = []
    
    tasks = if monitor_response_times do
      [Task.async(fn -> monitor_response_times(duration_ms) end) | tasks]
    else
      tasks
    end
    
    tasks = if monitor_error_rates do
      [Task.async(fn -> monitor_error_rates(duration_ms) end) | tasks]
    else
      tasks
    end
    
    tasks = if monitor_resource_usage do
      [Task.async(fn -> monitor_resource_usage(duration_ms) end) | tasks]
    else
      tasks
    end
    
    tasks = if monitor_process_counts do
  ...
  defp execute_load_requests(devices, interval_ms, end_time, total_requests, errors) do
    ...
    if current_time >= end_time do
      %{
        ...
        errors: errors,
        actual_rps: total_requests / ((end_time - (end_time - System.monotonic_time(:millisecond))) / 1000)
      }
    else
  ...
  defp execute_endurance_cycles(patterns, start_time, end_time, inject_failures, failure_rate, results) do
    current_time = System.monotonic_time(:millisecond)
    ...
    if current_time >= end_time do
      ...
      if final_results.system_crashed do
        ...
        Process.sleep(1000)  # Brief pause between cycles
        execute_endurance_cycles(patterns, start_time, end_time, inject_failures, failure_rate, final_results)
      end
    end
  end
  ...
  defp create_memory_pressure(intensity) do
    size = case intensity do
      :low -> 10_000_000    # 10MB
      :medium -> 50_000_000 # 50MB
      :high -> 100_000_000  # 100MB
    end
    
    spawn(fn ->
      # Create memory pressure by allocating large binaries
      _large_binary = :binary.copy(<<0>>, size)
      Process.sleep(10_000)  # Hold memory for 10 seconds
    end)
  end
  ...
  defp exhaust_ports(percentage) when percentage > 0 and percentage <= 1 do
    ...
    spawn(fn ->
      ...
      _ports = Enum.map(1..target_ports, fn _i ->
      ...
      Process.sleep(30_000)  # Hold ports for 30 seconds
    end)
  end
  ...
  defp inject_network_delays(_delay_ms) do
    ...
    spawn(fn ->
      ...
      receive do
        :stop -> :ok
      after
        60_000 -> :ok  # Auto-stop after 1 minute
      end
    end)
  end
  ...
  defp trigger_cascading_failures(failure_rate) do
    ...
    device_processes = Process.registered()
