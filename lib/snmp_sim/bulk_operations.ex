defmodule SnmpSim.BulkOperations do
  ...
  def estimate_response_size(varbinds) when is_list(varbinds) do
    base_overhead = 50  # SNMP message overhead
    
    varbind_size = 
      varbinds
      |> Enum.reduce(0, fn {oid, value, _behavior}, acc ->
        oid_size = byte_size(oid) + 10  # OID encoding overhead
        value_size = estimate_value_size(value)
        acc + oid_size + value_size + 8  # Variable binding overhead
      end)
  ...
  defp collect_repetitions(oid_tree, current_oid, remaining, acc) do
    ...
    case results do
      ...
      results when is_list(results) ->
        ...
        formatted_results = Enum.map(results, fn {oid, value, behavior} -> {oid, value, behavior} end)
  ...
  defp estimate_result_size({oid, value, _behavior}) do
    oid_size = byte_size(oid) + 10  # OID encoding overhead
    value_size = estimate_value_size(value)
    oid_size + value_size + 8  # Variable binding overhead
  end
  ...
  defp estimate_value_size(_), do: 8  # Default estimate
end