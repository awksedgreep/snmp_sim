defmodule SnmpSimEx.TestScenarios do
  ...
  def network_outage_scenario(devices, opts \\ []) do
    ...
    conditions = case recovery_type do
      :immediate ->
  ...
  def signal_degradation_scenario(devices, opts \\ []) do
    ...
    conditions = apply_signal_degradation(devices, %{
  ...
  def high_load_scenario(devices, opts \\ []) do
    ...
    Logger.info("Starting high load scenario #{scenario_id} with #{utilization_percent}% utilization")
    
    conditions = apply_high_load_conditions(devices, %{
  ...
  def device_flapping_scenario(devices, opts \\ []) do
    ...
    conditions = apply_flapping_conditions(devices, %{
  ...
  def cascading_failure_scenario(devices, opts \\ []) do
    ...
    conditions = apply_cascading_failure(devices, %{
    ...
    estimated_duration = calculate_cascade_duration(length(devices), 
      initial_failure_percentage, cascade_growth_factor, 
      max_affected_percentage, cascade_delay_seconds)
  ...
  def environmental_scenario(devices, opts \\ []) do
    ...
    conditions = apply_environmental_conditions(devices, %{
  ...
  def multi_scenario_test(devices, scenarios) do
    ...
    Logger.info("Starting multi-scenario test #{multi_scenario_id} with #{length(scenarios)} scenarios")
  ...
  defp apply_gradual_outage(devices, duration_seconds) do
    ...
    recovery_spread = duration_seconds * 0.3  # 30% spread in recovery times
    ...
    |> Enum.map(fn {device, index} ->
      ...
      outage_delay = index * 5 * 1000  # 5 seconds between failures
  ...
  defp apply_degrading_flapping(injector, config, index) do
    ...
    schedule_degrading_flaps(injector, initial_interval, config.down_duration_seconds * 1000, 
                           config.total_duration_ms, index)
  end
  ...
  defp apply_environmental_conditions(devices, config) do
    affected_devices = case config.geographic_pattern do
      :random ->
  ...
  defp calculate_cascade_duration(total_devices, initial_percentage, growth_factor, 
                                 max_percentage, delay_seconds) do
  ...
  defp schedule_flap_cycle(injector, interval, down_duration, total_duration) when total_duration > 0 do
    ...
    if next_total > interval do
      Process.send_after(self(), {:schedule_next_flap, injector, interval, down_duration, next_total}, 
  ...
  defp schedule_degrading_flaps(injector, current_interval, down_duration, remaining_duration, wave) do
    if remaining_duration > current_interval do
      ...
      next_interval = trunc(current_interval * 0.8)  # 20% faster each time
      next_remaining = remaining_duration - current_interval - down_duration
      
      Process.send_after(self(), {:schedule_degrading_flap, injector, next_interval, 
                                down_duration, next_remaining, wave + 1}, 
  ...
  defp schedule_cascade_waves(remaining_devices, config, current_count, max_affected, wave) do
    ...
    if devices_to_fail > 0 do
      ...
      Process.send_after(self(), {:cascade_wave, remaining_devices, devices_to_fail, wave + 1}, delay)
  ...
  defp apply_power_effects(injector, :moderate, index) do
    ...
    reboot_delay = index * 10000  # 10 seconds apart
