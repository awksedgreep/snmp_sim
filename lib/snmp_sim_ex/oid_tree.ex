defmodule SnmpSimEx.OIDTree do
  ...
  defstruct [
    :root,        # Root node of the OID tree
    :size,        # Number of OIDs in the tree
    :sorted_oids  # Cached sorted OID list for traversal
  ]
  ...
  defmodule Node do
    @moduledoc false
    defstruct [
      :oid_part,     # This part of the OID (integer)
      :value,        # Value for this OID (if it's a leaf)
      :behavior,     # Behavior configuration for this OID
      :children,     # Map of child nodes
      :is_leaf       # Whether this node represents a complete OID
    ]
  end
  ...
  defp build_sorted_oids(%Node{} = node, current_path) do
    ...
    child_oids = 
      node.children
      |> Enum.sort_by(fn {part, _} -> part end)  # Sort by OID part numerically
