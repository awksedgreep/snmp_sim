defmodule SNMPSimEx.Config do
  ...
  def load_yaml(file_path) when is_binary(file_path) do
    if Code.ensure_loaded?(YamlElixir) do
      ...
      {:error, {:missing_dependency, "yaml_elixir package not found. Add {:yaml_elixir, \"~> 2.9\"} to your deps."}}
    end
  end
    ...
    config = %{
      snmp_sim_ex: %{
        global_settings: %{
          ...
          enable_performance_monitoring: get_env_boolean("SNMP_SIM_EX_ENABLE_PERFORMANCE_MONITORING", true),
    ...
    case Enum.find(required_fields, fn field -> not Map.has_key?(group, field) end) do
      nil ->
        ...
        case group.port_range do
          %{start: start_port, end: end_port} when is_integer(start_port) and is_integer(end_port) ->
  ...
  defp start_device_group(group) do
    ...
    Logger.info("Successfully started #{length(successful_devices)}/#{count} devices for group #{group[:name]}")
    successful_devices
  end
