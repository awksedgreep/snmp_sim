defmodule SnmpSimEx.ErrorInjector do
  ...
  defstruct [
    :device_pid,
    :device_port,
    :error_conditions,    # Map of active error conditions
    :error_statistics,    # Statistics tracking
    :burst_state,         # Current burst error state
    :schedule_timers      # Scheduled error timers
  ]
  ...
  def handle_call({:remove_error, error_type}, _from, state) do
    ...
    updated_timers = Enum.reduce(removed_conditions, state.schedule_timers, fn {error_id, _config}, timers ->
  ...
  defp schedule_burst_timers(config, error_id, timers) do
    ...
    timer_ref = Process.send_after(self(), {:scheduled_error, error_id, :activate_burst}, burst_interval)
  ...
  defp schedule_recovery_timer(config, error_id, timers) do
    ...
    timer_ref = Process.send_after(self(), {:scheduled_error, error_id, :device_recovery}, config.duration_ms)
  ...
  defp handle_burst_activation(error_id, config, state) do
    ...
    # Update burst state
    updated_burst = Map.put(state.burst_state, error_id, %{
      active: true,
      start_time: DateTime.utc_now(),
      packets_affected: 0
    })
    ...
    timer_ref = Process.send_after(self(), {:scheduled_error, error_id, :deactivate_burst}, burst_duration)
    
    updated_timers = Map.put(state.schedule_timers, "#{error_id}_deactivate", timer_ref)
    
    # Update statistics
    updated_stats = %{state.error_statistics |
